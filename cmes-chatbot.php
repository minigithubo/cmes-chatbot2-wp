...
add_action('rest_api_init', function () {
  ...
  // 2b) Demo / contact sales → send to contact page
  if (cmes_wants_contact_or_demo($message)) {
    $contact_url = 'https://www.cmesrobotics.ai/en/contact/contact';
    return cmes_response(
      'For demo requests, quotes, or to contact sales, please use our contact form: ' . $contact_url,
      [],
      true,
      'contact'
    );
...
function cmes_wants_contact_or_demo($text) {
  $t = mb_strtolower((string)$text);
  $keywords = [
    'request a demo', 'request demo', 'schedule a demo', 'book a demo', 'get a demo',
    'contact sales', 'talk to sales', 'speak to sales', 'sales team', 'contact us',
    'request a quote', 'request quote', 'get a quote', 'get quote',
    'get in touch', 'reach out', 'reach us', 'contact form', 'how to contact',
  ];
...
function cmes_call_rag_module($payload) {
  ...
  if (is_wp_error($response)) {
    return [
      'answer' => 'Bot offline. Contact operator to turn on.',
      'sources' => [],
    ];
  }
  ...
  if (!is_array($body) || !isset($body['answer'])) {
    $is_question = mb_strlen(trim((string)($payload['message'] ?? ''))) >= 3;
    if ($is_question) {
      return [
        'answer' => 'Bot offline. Contact operator to turn on.',
        'sources' => [],
    ...
    return [
      'answer' => 'I couldn’t answer that. Please try rephrasing.',
      'sources' => [],
    ];
  }
